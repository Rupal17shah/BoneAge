...
input = Input(shape(480,480,3), name='in1')
...
x = MaxPooling2D(512, pool_size(3,3))(x)
x = Flatten()(x)
...
checkpoint = keras.callbacks.ModelCheckpoint(filepath='./weights/weights-regression.{epoch:02d}-{val_loss:.2f}.hdf5', save_weights_only=True, period=30)
...
model.save_weights("model-regression.h5")
with open('./data/history-regression.pkl', 'wb') as f:
